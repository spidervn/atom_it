...
#define CHECK_STAUS(STATUS, NAME)\
	if (STATUS != HPP_STATUS_NO_ERROR) { printf("%s error %d\n", NAME, STATUS); }\
	if (virtMatrix) {hppStatus delStatus = hppiDeleteVirtualMatrices(accel, virtMatrix); CHECK_DEL_STATUS(delSts, "hppiDeleteVirtualMatrices");}\
...
const char* keys = 
{
	...
	"{a accel |auto |accelerator type: auto (deault), cpu, gpu}"
};

// this is a sample for hppiSoble functions 
int main(int argc, const char** argv)
{
	...
	CHECK_STAUS(sts, "hppCreateInstance");
