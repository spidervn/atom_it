...
int main(int arg, char** argv)
{
	...
	Mat I = cv::cvarrToMat(IplI);	// Convert to the new style container
#else
	...
	split(I_YUV, planes);	// Split the image into seperate color planes (Y U V)
	//! [new]	

#if 1	// Change it to 0 if you want to see a blurred and noisy version 
	//! [scanning]
	// Mat scanning
	...
	{
		// Method 2. process the first chroma plance using pre-stored row pointer
		uchar* Uptr = planes[1].ptr<uchar>(y);
		for (int x = 0; x < I_YUV.cols; x++)
		{
			Uptr[x] = saturate_cast<uchar>((Uptr[x]-128)/2 + 128);
		
			// Method 3. process the 2nd chroma plane using individual element access
	...
	// step 2) call the function (tip: to pass a pointer do not forget unary "&" to form pointer)
	
	IplImage cv_planes_0 = planes[0], cv_noise = noisyI;
	cvAddWeighted(&cv_planes[0], contrast_gain, &cv_noise, 1, -128 + brightness_gain, &cv_planes_0);
#else
	addWeighted(planes[0], contrast_gain, noisyI, 1, -128 + brightness_gain, planes[0]);
#endif
	
	const double color_space = 0.5;
	...
	// Mat::mul replaces cvMul(). Again, no temporary arrays are created in case of simple expression
